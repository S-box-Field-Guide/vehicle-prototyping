...
public static class TestTrack
{
	...
	// Slalom cone registry (authored transforms), cleared and rebuilt every Build.
	// WHY: cones are DYNAMIC (punt-able, city polish pass) but the battery's strike
	// counter (SlalomManeuver.CountConeStrikes) is proximity-based against live
	// "Slalom Cone" objects, i.e. layout-relative. A cone left displaced by one run
	// would corrupt the NEXT run's course, so each maneuver start teleports every
	// cone back to its authored spot via ResetSlalomCones. Same static-registry
	// idiom as Stations: populated by Build, empty until then.
	static readonly List<(GameObject go, Vector3 pos, Rotation rot)> _slalomCones = new();
	...
	{
		var go = Cone( posMeters, name );
		if ( go is null )
		{
			go = Block( posMeters + Vector3.Up * 0.6f, new Vector3( 0.6f, 0.6f, 1.2f ), fallbackColor, collide: true, name: name );
			MakeConeDynamic( go );
		}
		_slalomCones.Add( (go, go.WorldPosition, go.WorldRotation) );
	}

	/// <summary>
	/// Teleports every slalom cone back to its authored build-time transform and kills its
	/// momentum. Called from SlalomManeuver.Start: cones are dynamic (punt-able), but the
	/// strike counter is proximity-based against the authored layout, so a run must always
	/// start from the identical cone arrangement (see _slalomCones). Safe no-op before Build.
	/// </summary>
	public static void ResetSlalomCones()
	{
		foreach ( var c in _slalomCones )
		{
			if ( !c.go.IsValid() )
				continue;
			c.go.WorldPosition = c.pos;
			c.go.WorldRotation = c.rot;
			var rb = c.go.Components.Get<Rigidbody>();
			if ( rb.IsValid() )
			{
	...
	/// <summary>
	/// Flips a cone's collider non-static and gives it a light Rigidbody, so a car PUNTS it
	/// instead of hitting a fixed post (city polish pass, cones were car-stopping). ~4 kg:
	/// a real cone's order of weight, light enough that any car sends it flying, heavy
	/// enough not to jitter at rest. Damping 0.5/0.5 so punted cones settle instead of
	/// rolling forever.
	/// </summary>
	static void MakeConeDynamic( GameObject go )
	{
	...
	/// ground (graceful null-on-missing-model fallback; collider fit to model bounds, NOT
	/// pre-multiplied by scale, since BoxCollider follows the GameObject's WorldScale).
	/// Cones are rotationally symmetric so no yaw parameter is needed. Dynamic: see
	/// <see cref="MakeConeDynamic"/>.
	/// </summary>
	static GameObject Cone( Vector3 posMeters, string name )
	{
