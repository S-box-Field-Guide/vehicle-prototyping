...
| `Tune` | T | (none) | expand/collapse the demo live tuning lab (demo scene only) |
| `CyclePrev` / `CycleNext` | [ / ] | D-pad left / right | switch between the demo cars (demo scene only) |
...
minute. A small `T tuning` chip marks it top-left from spawn; press `Tune` (T) to expand it (the
camera hands you the cursor) and again to collapse. Hop between the four cars with `[` and `]`
(the top-right legend reminds you). It binds to the car the
